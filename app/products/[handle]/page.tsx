...
async function getProduct(handle: string): Promise<ProductData | null> {
  if (!r2Base) return null;

  try {
    const folder = productFolder(handle);
    const res = await fetch(
      `${r2Base.replace(/\/$/, "")}/catalog/products/${folder}/${handle}.json`,
      { cache: "force-cache" }
    );
...
function getProductImage(product: ProductData) {
  ...
  const folder = product.collection || product.category || "";

  return `${r2Base.replace(/\/$/, "")}/catalog/images/${folder}/${image}`;
}

function getSeoData(product: ProductData, fallbackHandle: string) {
  const variant = product.variants?.[0];

  const partNumber = cleanText(product.handle || fallbackHandle);

  const productTitle = cleanProductTitle(
    variant?.title || product.title || partNumber
  );

  const brand = cleanText(variant?.vendor);
  const category = titleFromHandle(product.collection || product.category);

  const replacements =
    product.variants
      ?.map((item) => cleanProductTitle(item.title))
      .filter(Boolean)
      .join(", ") || "";
...
export async function generateMetadata({
  ...
  const fullTitle = `${seo.metaTitle} | Sparesco`;
  ...
  const productJsonLd = product
    ? (() => {
        ...
        return {
          ...
          sku: seo.partNumber.toUpperCase(),
          brand: seo.brand
            ? {
                "@type": "Brand",
                name: seo.brand,
          ...
          offers: {
            ...
            priceCurrency: "INR",
            price:
              seo.variant?.price && seo.variant.price > 0
                ? String(seo.variant.price)
                : undefined,
