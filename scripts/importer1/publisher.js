...
async function publishAll(products) {
  ...
  for (let index = 0; index < catalogPages; index++) {
    catalogJobs.push({
      key:
        `catalog/indexes/catalog-pages/` +
        `${pageNumber(index)}.json`,
      data: catalog.slice(
        index * config.PAGE_SIZE.catalog,
        (index + 1) * config.PAGE_SIZE.catalog
      ),
    });
  }

  await uploadJobs("Catalog", catalogJobs);

  const categoryGroups = new Map();

  for (const item of catalog) {
    for (const tag of item.tags || []) {
      if (!categoryGroups.has(tag)) {
        categoryGroups.set(tag, []);
      }
  ...
  for (const [handle, items] of categoryGroups.entries()) {
    ...
    for (let index = 0; index < totalPages; index++) {
      categoryJobs.push({
        ...
        data: items.slice(
          index * config.PAGE_SIZE.category,
          (index + 1) * config.PAGE_SIZE.category
        ),
      });
    }
  }
  ...
  await uploadJobs("Search", searchJobs);
  ...
  await uploadJobs("Registry", registryJobs);

  const duplicateRegistryJobs =
    buildDuplicateRegistryJobs(products);

  await uploadJobs(
    "Duplicate registry",
    duplicateRegistryJobs
  );
