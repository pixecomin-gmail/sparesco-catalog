...
async function publishAll(products) {
  ...
  const searchJobs = [
    {
      key:
        "catalog/indexes/search-index.json",
      data:
        searchIndex,
    },
  ];
  ...
  for (
    const [key, data]
    of registryShards.entries()
  ) {
    registryJobs.push({
      key:
        `catalog/handles/${key}.json`,
      data,
    });
  }
